...
- **Tích hợp RBAC Firestore Query theo Vai Trò (Role-Based Access Control):** Nâng cấp `FirebaseTransactionRepository` hỗ trợ phân quyền vai trò người dùng (`roleId`). Nếu người dùng thuộc nhóm quản lý/phê duyệt (`admin`, `chiefAccountant`, `accountant`, `manager`), câu truy vấn Firestore sẽ tải toàn bộ danh sách giao dịch của toàn công ty. Ngược lại, nếu là nhân viên cá nhân (`salesperson`), hệ thống sẽ lọc theo `userId` riêng.
- **Merge & Đồng bộ Nhánh Main:** Merge thành công nhánh `origin/main` mới nhất từ Trưởng nhóm vào `feature/transactions-ui-form`, giải quyết toàn bộ xung đột, đảm bảo `flutter analyze` đạt 0 lỗi.
